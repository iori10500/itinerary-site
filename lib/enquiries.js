...
async function sendResend(enquiry) {
  ...
  const cc = (process.env.ENQUIRY_EMAIL_CC || 'steven.shan@wildroadgroup.com').split(',').map(v => v.trim()).filter(Boolean);
  const payload = {
    from: process.env.ENQUIRY_EMAIL_FROM || 'WR Journeys <info@wildroadgroup.com>',
